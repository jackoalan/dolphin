...
RenderParent::RenderParent(RenderWidget* render_widget, QWidget* parent)
: QStackedWidget(parent), m_render_widget(render_widget)
{
  setAttribute(Qt::WA_NativeWindow);
  setCurrentIndex(addWidget(render_widget));
  connect(Host::GetInstance(), &Host::RequestTitle, this, &QWidget::setWindowTitle);
}

bool RenderParent::event(QEvent* event)
{
  switch (event->type())
  {
  case QEvent::Close:
    m_render_widget->event(event);
    break;
  default:
    break;
  }
